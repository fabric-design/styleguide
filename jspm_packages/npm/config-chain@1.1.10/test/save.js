/* */ 
var CC = require('../index').ConfigChain;
...
test('test saving and loading ini files', function(t) {
  new CC().add({grelb: 'blerg'}, 'opt').addFile(f1, 'ini', 'inifile').addFile(f2, 'json', 'jsonfile').on('load', function(cc) {
    t.same(cc.snapshot, {
      grelb: 'blerg',
      bloo: 'jaus',
      foo: {bar: 'baz'},
    ...
    cc.set('grelb', 'brelg', 'opt').set('foo', 'zoo', 'inifile').set('oof', 'ooz', 'jsonfile').save('inifile').save('jsonfile').on('save', function() {
